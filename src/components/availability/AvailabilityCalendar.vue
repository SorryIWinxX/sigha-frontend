    ...
    <div class="w-full h-full flex flex-col overflow-hidden bg-white">
        ...
        <div class="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4 flex-shrink-0">
            ...
            <div class="flex flex-wrap gap-2 w-full md:w-auto">
                ...
                <Button variant="info" :disabled="!props.semesterAvailability" @click="openPreviousAvailabilityModal">
        ...
        <!-- Modal para disponibilidad anterior -->
        <PreviousAvailabilityModal :is-open="showPreviousAvailabilityModal" :current-semester-id="props.semesterId"
            @close="closePreviousAvailabilityModal" @availability-loaded="handlePreviousAvailabilityLoaded" />
    </div>
</template>
...
import PreviousAvailabilityModal from './PreviousAvailabilityModal.vue';
...
const showPreviousAvailabilityModal = ref(false);
...
function openPreviousAvailabilityModal() {
    showPreviousAvailabilityModal.value = true;
}

function closePreviousAvailabilityModal() {
    showPreviousAvailabilityModal.value = false;
}

function handlePreviousAvailabilityLoaded(data) {
    const { slots, semesterDescription } = data;

    // Filtrar slots que están aprobados o rechazados (no se pueden reemplazar)
    const protectedSlots = internalSelectedSlots.value.filter(slot => {
        const status = slot.statusId;
        return status === 2 || status === 3; // Aprobado o Rechazado
    });

    // Combinar los slots protegidos con los nuevos slots
    const newSlots = [...protectedSlots];

    // Agregar slots de disponibilidad anterior que no entren en conflicto con slots protegidos
    slots.forEach(slot => {
        const isProtected = protectedSlots.some(
            protectedSlot => protectedSlot.day === slot.day && protectedSlot.hour === slot.hour
        );
    ...
    internalSelectedSlots.value = newSlots;
    saveToHistory();

    showSuccessToast(`Disponibilidad cargada desde ${semesterDescription}. Los horarios ya aprobados o rechazados se mantuvieron sin cambios.`);
}
